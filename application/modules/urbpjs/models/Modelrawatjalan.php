<?php
    class Modelrawatjalan extends CI_Model{
            ...
            $query =
                    "
                        ...
                        SELECT
                            ...
                            SUM(CASE
                                WHEN B.SEP_NOMOR IS NULL THEN 0
                                ELSE 1
                            END) AS JUMLAH_SEP,

                            SUM(CASE
                                WHEN B.SEP_NOMOR IS NULL THEN 1
                                ELSE 0
                            END) AS JUMLAH_UN_SEP,

                            SUM(CASE
                                WHEN C.CODING_SOURCE='GROUPING' THEN 1
                                ELSE 0
                            END) AS JUMLAH_GROUPING,

                            SUM(CASE
                                WHEN B.SEP_NOMOR IS NOT NULL
                                AND NVL(C.CODING_SOURCE,'BELUM') <> 'GROUPING'
                                THEN 1
                                ELSE 0
                            END) AS JUMLAH_UN_GROUPING,

                            SUM(CASE
                                WHEN B.SEP_NOMOR IS NULL
                            ...
                            SUM(CASE
                                ...
                                THEN CASE
                                    ...
                                    THEN NVL(P.TARIF_RS_REAL,0)
                                    ELSE NVL(C.TARIF_RS,0)
                                END
                                ELSE 0
                            END) AS NILAI_UN_GROUPING,

                            SUM(CASE
                                WHEN C.CODING_ID IS NULL
                                THEN NVL(P.TARIF_RS_REAL,0)
                                ELSE C.TARIF_RS
                            END) AS TOTAL_TARIF_RS,

                            SUM(CASE
                                WHEN C.CODING_SOURCE='GROUPING'
                                THEN NVL(C.TARIF_INACBG,0)
                                ELSE 0
                            END) AS NILAI_GROUPING,
            ...
            $query =
                    "
                        ...
                        SELECT
                            ...
                            B.SEP_NOMOR,
                            B.LASTUPDATE,
                            (SELECT UPPER(NAMA) FROM SR01_MED_DOKTER_MS WHERE LOKASI_ID='001' AND DOKTER_ID=A.DOKTER_ID) AS NAMADOKTER,
                            (SELECT KETERANGAN FROM SR01_MED_POLI_MS WHERE LOKASI_ID='001' AND AKTIF='1' AND POLI_ID=A.POLI_ID) AS POLIKLINIK,
                            SR01_GET_SUFFIX(A.PASIEN_ID) AS NAMAPASIEN,
                            GETPIDINT(A.PASIEN_ID) AS MRPAS
                        FROM SR01_KEU_EPISODE A
                        LEFT JOIN BPJS B ON B.PASIEN_ID=A.PASIEN_ID AND B.EPISODE_ID=A.EPISODE_ID
                        WHERE A.LOKASI_ID='001'
                        AND A.AKTIF='1'
                        AND A.JENIS_EPISODE='O'
                        AND A.STATUS_EPISODE<>'99'
                        AND A.REKANAN_ID='BPJS'
                        AND TO_CHAR(A.TGL_MASUK,'YYYY')='".$periode."'
                        AND A.POLI_ID NOT IN ('APS','APS L0000000001','APS R0000000001','RUJUKANLUAR')
                        AND (
                            A.POLI_ID IN ('POLIFISIO','POLIFISOKUP','POLIFISWICARA','HEMOD0000000000','CAPD0000000001','UGD01','UGD02')
                            OR EXISTS (
                                SELECT 1
                                FROM SR01_MED_PRWT_TR T
                                WHERE T.LOKASI_ID='001'
        ...
        function datadetailungrouping($periode){
            $query =
                    "
                        SELECT
                            A.PASIEN_ID,
                            ...
                            C.BAST,
                            C.BAHV,
                            NVL(C.BILL_TOTAL,0) AS TARIF_RS,
                            NVL(C.TARIF_INACBG,0) AS TARIF_INACBG,
                            NVL(C.TARIF_INACBG,0) - NVL(C.BILL_TOTAL,0) AS SELISIH,
