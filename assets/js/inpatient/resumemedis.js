let tableresume       = null;
let tableresumedokter = null;
...
function resumemedis(){
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            const tableresume       = initDataTable("#datapendingresume_table","#searchtable",100);
            const tableresumedokter = initDataTable("#datapendingresumedokter_table","#searchtable",10);
...
function filterDokter(namaDokter) {
	const tabTrigger = document.querySelector('a[href="#tabraw"], button[data-bs-target="#tabraw"]');

	if (tabTrigger) {
		bootstrap.Tab.getOrCreateInstance(tabTrigger).show();
	}

	const table = $("#datapendingresume_table").DataTable();

	$("#searchtable").val(namaDokter);

	table.search(namaDokter).draw();
